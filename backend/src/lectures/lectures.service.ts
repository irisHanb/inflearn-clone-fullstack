...
export class LecturesService {
  ...
  async create(
    ...
    if (!section) {
      return new NotFoundException('섹션을 찾을 수 없습니다.');
    }
  ...
  async update(lectureId: string, updaetLectureDto: UpdateLectureDto, userId) {
    ...
    if (lecture.course.instructorId !== userId) {
      return new UnauthorizedException('이 강의를 수정할 권한이 없습니다.');
    }
  ...
  async findOne(lectureId: string, userId: string) {
    ...
    if (lecture.course.instructorId !== userId) {
      return new UnauthorizedException('이 강의를 조회할 권한이 없습니다.');
    }
    return lecture;
  }
  ...
  async remove(lectureId: string, userId: string) {
    ...
    if (lecture.course.instructorId !== userId) {
      return new UnauthorizedException('이 강의를 삭제할 권한이 없습니다.');
    }
